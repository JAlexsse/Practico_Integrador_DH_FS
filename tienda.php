<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">

  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css"
    integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
  <link href="https://fonts.googleapis.com/css?family=Bebas+Neue|Bitter|Montserrat&display=swap" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css?family=Montserrat:400,500,700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="css/fonts.css">
  <link rel="stylesheet" href="css/stylehome.css">

  <title>Tienda</title>
</head>

<body>

  <?php include 'php/header.php'; ?>

  <main class="container-fluid my-3 main-tienda">
    <div class="row justify-content-center">

      <div class="col-12 bg-light text-center py-2 border">
        <h6 class="titulos-importantes">Tienda</h6>
      </div> <!-- Titulo -->

      <div class="col-12 col-lg-2  m-2 text-left border-right ">
        <h3 class="mt-2 titulos-medio">Categorias</h3>

        <div id="acordion" role="tablist" aria-multiselectable="true">

          <div class="card p-0">
            <div class="card-header" role="tab" id="heading1">
              <h5 class="mb-0 titulos-categorias-tienda">
                <a href="#collapse1" data-toggle="collapse" data-parent="#acordion" aria-expanded="true"
                  aria-controls="collapse1">
                  Bicicletas
                </a>
              </h5>
            </div>
            <!--fin div card-header -->

            <div id="collapse1" class="collapse show" role="tabpanel" aria-labelledby="heading1">
              <div class="card-block">
                <ul class="nav flex-column">
                  <li class="nav-item">
                    <a class="nav-link active hov-bici categorias-tienda" href="#">Adventure</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-bici categorias-tienda" href="#">Road</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-bici categorias-tienda" href="#">City</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-bici categorias-tienda" href="#">Cruisers</a>
                  </li>
                </ul>
              </div>

            </div>
          </div>
          <!-- Bicicletas -->

          <div class="card">
            <div class="card-header" role="tab" id="heading2">
              <h5 class="mb-0 titulos-categorias-tienda">
                <a href="#collapse2" data-toggle="collapse" data-parent="#acordion" aria-expanded="true"
                  aria-controls="collapse2">
                  Accesorios
                </a>
              </h5>
            </div>
            <!--fin div card-header -->

            <div id="collapse2" class="collapse" role="tabpanel" aria-labelledby="heading2">
              <div class="card-block">
                <ul class="nav flex-column">
                  <li class="nav-item">
                    <a class="nav-link active hov-acce categorias-tienda" href="#">Protector Bicicletas</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-acce categorias-tienda" href="#">Guardabarros</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-acce categorias-tienda" href="#">Botellas</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-acce categorias-tienda" href="#">Luces</a>
                  </li>
                </ul>
              </div>

            </div>
          </div><!-- Accesorios -->

          <div class="card">
            <div class="card-header" role="tab" id="heading3">
              <h5 class="mb-0 titulos-categorias-tienda">
                <a href="#collapse3" data-toggle="collapse" data-parent="#acordion" aria-expanded="true"
                  aria-controls="collapse3">
                  Partes
                </a>
              </h5>
            </div>
            <!--fin div card-header -->

            <div id="collapse3" class="collapse" role="tabpanel" aria-labelledby="heading3">
              <div class="card-block">
                <ul class="nav flex-column">
                  <li class="nav-item">
                    <a class="nav-link active hov-part categorias-tienda" href="#">Pedales</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-part categorias-tienda" href="#">Asientos</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-part categorias-tienda" href="#">Cuadro</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-part categorias-tienda" href="#">Frenos</a>
                  </li>
                </ul>
              </div>

            </div>
          </div>
          <!-- Partes -->

          <div class="card">
            <div class="card-header" role="tab" id="heading4">
              <h5 class="mb-0 titulos-categorias-tienda">
                <a href="#collapse4" data-toggle="collapse" data-parent="#acordion" aria-expanded="true"
                  aria-controls="collapse4">
                  Cubiertas
                </a>
              </h5>
            </div>
            <!--fin div card-header -->

            <div id="collapse4" class="collapse" role="tabpanel" aria-labelledby="heading4">
              <div class="card-block">
                <ul class="nav flex-column">
                  <li class="nav-item">
                    <a class="nav-link active hov-cubrt categorias-tienda" href="#">Adventure</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-cubrt categorias-tienda" href="#">Road</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-cubrt categorias-tienda" href="#">City</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link hov-cubrt categorias-tienda" href="#">Cruisers</a>
                  </li>
                </ul>
              </div>

            </div>
          </div>
          <!-- Cubierta -->

        </div>
      </div> <!-- Filtros -->

      <div class="col">
        <div class="row">
          <div class="col produc">
            <div class="row justify-content-center">

              <div class="col-10 col-md-4 text-center p-3 mb-3">
                <div class="row item mx-2">
                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/BICICLETAS/ciudad.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: #88280a;">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                        <label class="col-2 btn btn-secondary" style="background-color: #000;">
                          <input type="radio" name="options" id="option2">
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Bicicleta 1</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>
              </div> <!-- bicis 1 -->

              <div class="col-10 col-md-4  text-center p-3 mb-3">
                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center " style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/BICICLETAS/carretera.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: #42016d;">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Bicicleta 2</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- bicis 2 -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">
                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="">
                      <img src="img/PRODUCTOS/BICICLETAS/aventura.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: #000;">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                        <label class="col-2 btn btn-secondary" style="background-color: #9c9c9c;">
                          <input type="radio" name="options" id="option2">
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Bicicleta 3</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- bicis 3 -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/EQUIPO Y ACCESORIOS/casco1.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: rgb(160, 137, 7);">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                        <label class="col-2 btn btn-secondary" style="background-color: #1d2450;">
                          <input type="radio" name="options" id="option2">
                        </label>
                        <label class="col-2 btn btn-secondary" style="background-color: #000000;">
                          <input type="radio" name="options" id="option2">
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Casco 1</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- casco1 -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/EQUIPO Y ACCESORIOS/casco2.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: #000;">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                        <label class="col-2 btn btn-secondary" style="background-color: #ffffff;">
                          <input type="radio" name="options" id="option2">
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Casco 1</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!--  casco2 -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/EQUIPO Y ACCESORIOS/casco3.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: rgb(231, 209, 8);">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Casco 3</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- Casco3 -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <a href="item.html">
                      <img src="img/PRODUCTOS/REPUESTOS/Pedal1.jpg" class="card-img-top" alt="...">
                      <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                        <label class="col-2 btn btn-secondary active" style="background-color: rgb(20, 20, 20);">
                          <input type="radio" name="options" id="option1" checked>
                        </label>
                      </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Pedal</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- Pedal -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <img src="img/PRODUCTOS/REPUESTOS/Asiento.jpeg" class="card-img-top" alt="...">
                    <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                      <label class="col-2 btn btn-secondary active" style="background-color: rgb(219, 158, 25);">
                        <input type="radio" name="options" id="option1" checked>
                      </label>
                    </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Asiento</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- Asiento -->

              <div class="col-10 col-md-4 text-center p-3 mb-3">

                <div class="row item mx-2">

                  <div class="col-12 mt-3">
                    <div class="row item-header justify-content-between align-items-center" style="height: 30px;">
                      <div class="col-4 d-flex">
                        <span class="badge badge-success mr-1">Stock</span>
                        <span class="badge badge-info">Nuevo</span>
                      </div>
                      <div class="col-2 fav">
                        <img src="img/ICONOS/heart.svg" alt="Favorito">
                      </div>
                    </div>
                  </div> <!-- item header -->

                  <div class="col-12">
                    <img src="img/PRODUCTOS/CUBIERTAS/WheelSet_Gold_700c_40_3rd_web_1200x.jpg" class="card-img-top"
                      alt="...">
                    <div class="row btn-group btn-group-toggle justify-content-center pb-2" data-toggle="buttons">
                      <label class="col-2 btn btn-secondary active" style="background-color: rgb(219, 168, 26);">
                        <input type="radio" name="options" id="option1" checked>
                      </label>
                    </div>
                    </a>
                  </div> <!-- imagen -->

                  <div class="col-12 my-3">
                    <h1 class="h5 titulos-categorias-tienda">Bicicleta 2</h1>
                    <h6 class="categorias-tienda">$999</h6>
                  </div> <!-- Informacion item -->
                </div>

              </div> <!-- Cubierta -->

            </div>
          </div> <!-- items -->

          <div class="col-12 border-top">
            <div class="row justify-content-between align-items-center">
              <div class="col">
                <p>Articulos 1-12 de 30</p>
              </div> <!-- Cantidad de Articulos -->
              <div class="col">
                <ul class="pagination justify-content-end">
                  <li class="page-item disabled">
                    <a class="page-link" href="#" tabindex="-1" aria-disabled="true">Previous</a>
                  </li>
                  <li class="page-item"><a class="page-link" href="#">1</a></li>
                  <li class="page-item"><a class="page-link" href="#">2</a></li>
                  <li class="page-item"><a class="page-link" href="#">3</a></li>
                  <li class="page-item">
                    <a class="page-link" href="#">Next</a>
                  </li>
                </ul>
              </div> <!-- Navegacion entre paginas -->
            </div>

          </div> <!-- Navegacion -->

        </div>
      </div> <!-- Productos -->

    </div>
  </main> <!-- container -->

  <?php include 'php/footer.php'; ?>



  <!--Script Bootstrap-->
  <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js"
    integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous">
  </script>
  <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js"
    integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous">
  </script>
  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"
    integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous">
  </script>

</body>

</html>